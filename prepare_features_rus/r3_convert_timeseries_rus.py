...
def convert_timeseries_countries(type):
    if type == 'confirmed':
        tbl = pd.read_csv(
            INPUT_PATH + 'COVID-19-master/csse_covid_19_data/csse_covid_19_time_series/time_series_covid19_confirmed_global.csv')
    else:
        tbl = pd.read_csv(
            INPUT_PATH + 'COVID-19-master/csse_covid_19_data/csse_covid_19_time_series/time_series_covid19_deaths_global.csv')
...
def convert_timeseries_us(type):
    if type == 'confirmed':
        tbl = pd.read_csv(
            INPUT_PATH + 'COVID-19-master/csse_covid_19_data/csse_covid_19_time_series/time_series_covid19_confirmed_US.csv')
    else:
        tbl = pd.read_csv(
            INPUT_PATH + 'COVID-19-master/csse_covid_19_data/csse_covid_19_time_series/time_series_covid19_deaths_US.csv')

    tbl['Combined_Key'].fillna('', inplace=True)
    tbl['name'] = tbl['Combined_Key']

    dates = list(tbl.columns.values)
    remove_feats = ['UID', 'iso2', 'iso3', 'code3', 'FIPS', 'Admin2', 'Province_State', 'Country_Region', 'Lat',
                    'Long_', 'Combined_Key', 'name', 'Population']
...
def convert_timeseries_all_regions_last_date():
    ...
    nor_found = 0
    for index, row in s.iterrows():
        name1, name2, prov, name3, name4, date, cases, deaths = row['name1'], row['name2'], row['Province_State'], row[
            'Country_Region'], row['ISO3'], row['Last_Update'], row['Confirmed'], row['Deaths']
